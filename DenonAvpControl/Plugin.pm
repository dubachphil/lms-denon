#	DenonAvpControl
#
#	Author:	Chris Couper <ccouper(at)fastkat(dot)com>
#	Credit To: Felix Mueller <felix(dot)mueller(at)gwendesign(dot)com>
#
#	Copyright (c) 2003-2008 GWENDESIGN, 2008-2012 Chris Couper
#	All rights reserved.
#
#	----------------------------------------------------------------------
...
#			- Set:AvpIP Address, On and Off Delays, Max Volume, Quickselect
#	----------------------------------------------------------------------
#	History:
#
...
#				Slim::Utils::Timers::killTimers( $client, \&handlePowerOn); 
	...
	# Get clients volume adjustment
		...
		if (! $selfInitiated) {
			...
			my $subVol = sprintf("%d",((80 + $maxVolume) * (sqrt($volAdjust)*10) / 10 ));
			my $digit = int(substr($subVol,2,1));
			$DenonVol = sprintf("%d",$subVol/10);
			if ($digit>2 && $digit<8) {
				$DenonVol = $DenonVol*10 + 5;
			}
...
#		&handleQuickSelect($client);
		Slim::Utils::Timers::setTimer( $client, (Time::HiRes::time() + 1), \&handleQuickSelect); 
	} else {
...
sub updateSqueezeVol { #used to sync SB vol with AVP
	...
	$log->debug("*** DenonAvpControl:max volume: $maxVolume \n");
	my $volAdjust = sprintf("%d",(($avpVol / (80 + $maxVolume)) * 10)**2);
